...
namespace Steamless.Classes
{
    ...
    public class Pe32Helpers
    {
        ...
        /// <returns></returns>
        public static NativeApi32.ImageSectionHeader GetSection(byte[] rawData, int index, NativeApi32.ImageDosHeader dosHeader, NativeApi32.ImageNtHeaders ntHeaders)
        {
            var sectionSize = Marshal.SizeOf(typeof(NativeApi32.ImageSectionHeader));
            var optionalHeaderOffset = Marshal.OffsetOf(typeof(NativeApi32.ImageNtHeaders), "OptionalHeader").ToInt32();
            var dataOffset = dosHeader.e_lfanew + optionalHeaderOffset + ntHeaders.FileHeader.SizeOfOptionalHeader;

            return GetStructure<NativeApi32.ImageSectionHeader>(rawData, dataOffset + (index * sectionSize));
        }
